/// The dmacro CLI as a library — the user-friendly entry point for projects
/// that depend on dmacro as a package.
///
/// A consumer project gets the full CLI (compile / watch / trace / repl /
/// --check) with **their own macros loaded** from a five-line entry point:
///
/// ```dart
/// // tool/dmacro.dart — in YOUR project
...
/// ```
///
/// Projects that only use the built-in macros don't need an entry point —
/// `dart run dmacro compile <file>` works directly because dmacro declares
/// an executable.
///
...
/// Runs the dmacro CLI with the given [args].
///
/// [registerMacros] is the hook for user-authored macros: it runs after the
/// built-in and schema macros are registered and before any compilation, so
/// macros it defines (via [defmacro] / `defAsyncMacro`) are available to every
/// compiled source — including overriding a built-in by registering the same
/// name. It may be async (e.g. to read config before registering).
///
/// Exits the process on fatal CLI errors (unknown command, stale `--check`),
/// matching standard CLI behavior — call it from a `main`, not from library
/// code that must survive the call.
Future<void> runDmacro(
  List<String> args, {
  FutureOr<void> Function()? registerMacros,
}) async {
  registerBuiltins();
  registerSchemaMacros();
  if (registerMacros != null) await registerMacros();
...
Future<void> _compileDir(String dir,
  ...
  if (macroSources.isEmpty && inlineDartFiles.isEmpty) {
    stderr.writeln(
        'No .dmacro, .sexp, or inline-block .dart files found under $dir');
    return;
  }
...
Future<bool> _compileSingle(String inputPath,
    ...
    bool doFormat = true,
    bool fieldOrigins = false}) async {
  // Normalize so the path embedded in the generated header / origin comments
  // does not depend on how the path was spelled on the command line.
  // Without this, `compile models.dmacro` then `compile . --check` reports
  ...
  // Generated code is never hand-edited, so don't subject it to the consumer's
  // style lints (e.g. prefer_single_quotes on string literals carried verbatim
  // from the source). This is standard practice for codegen output and keeps a
  // consumer's `dart analyze` clean. `type=lint` suppresses lints only — real
  // analyzer errors still surface.
...
/// Strips leading `./` segments so the same file always embeds the same
/// origin path regardless of command-line spelling or directory scanning.
...
Usage (in a project that depends on dmacro):
  dart run dmacro compile <file.dmacro>              Dart-like → Dart
  dart run dmacro compile <file.sexp>                S-expression → Dart
  dart run dmacro compile <file.dart>                Expand inline @@dmacro blocks
  dart run dmacro compile <file> -o output.dart      Write to specific file
  dart run dmacro compile <dir>                      Compile all sources under dir
  dart run dmacro compile <dir> --check              CI: exit non-zero if stale
  dart run dmacro compile <file> --no-format         Skip dart format
  dart run dmacro watch <path>                       Watch and recompile on save
  dart run dmacro watch <path> --with-analyze        Watch + dart analyze after each compile
  dart run dmacro trace <file>                       Print each macro expansion step (debug)
  dart run dmacro repl                               Interactive REPL

Custom macros — write your own generator (full CLI, your macros loaded):
  // tool/dmacro.dart
  import 'package:dmacro/dmacro.dart';
  void main(List<String> args) => runDmacro(args, registerMacros: () {
